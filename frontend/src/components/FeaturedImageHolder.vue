...
<div @mouseover="isHovered=true" @mouseleave="isHovered=false" class="holder holder-focus">
  <div v-if="isHovered" class="holder-top-right">
    <b-icon-x-circle-fill @click="processFeatured()" class="holder-icon-focus" font-scale="1.3" variant="danger"/>
  </div>
...
export default {
  ...
  methods: {
    processFeatured: function() {
      ...
      const data = {'remove': [this.img.id]};
      axios
        .post(`${this.$globals.backend}/api/featured/${this.$globals.flockEvent['userId']}/update/`, data, {headers: headers})
        .then(response => {
          console.log(response);
          this.$root.$emit('addToast','Good evening',`Deleted ${this.img.name} from featured`, 'success', 1000);
          this.$root.$emit('removeImage', this.img.id);
        })
        .catch(error => {
          this.$root.$emit('addToast', 'Good evening',`Image ${this.img.name} has not been removed: ${error}`, 'danger', 2000);
        })
